...
//    .map((frame :DataFrame) => frame.select("textTransformed", "prediction").as[(Array[String], Double)].rdd)
//    .flatMap((value: RDD[(Array[String], Double)]) => value.collect())
//    .foreachRDD((value: RDD[(Array[String], Double)], time: Time) => value.saveAsTextFile("output"))
